...
1. פרטי השיעור הכוללים את השדות (משני סוגים: טקסט, תיבות בחירה): 
	- קטגוריה (תיבת בחירה עם האפשרויות הבאות: מתמטיקה, אנגלית, עברית, תנ״ך, היסטוריה, אזרחות, ספרות, פיזיקה, כימיה, ביולוגיה, מדעים, גיאוגרפיה, מחשבים, אומנות, מוזיקה, חינוך גופני, פילוסופיה, פסיכולוגיה, סוציולוגיה, חינוך חברתי) 
	- נושא היחידה
	- זמן כולל 
	- שכבת גיל
	- ידע קודם נדרש
...
דוגמא למבנה תשובה בסיסי JSON לעדכון שדה מסוים (כל השדות החוזרים יהיו בתוך סוגריים מרובעים - מערך של אובייקטים בJSON חייב להחזיר את שלושת השדות גם אם אחד מהם ריק):
...
חייב תמיד להחזיר 3 שדות בדיוק בתשובה בדיוק בשמות שמוגדרים (opening.0.screen1 - במקרה זה מדובר על שדה מסך 1 ,userResponse - שדה תגובה אנושית בנוגע לשדה המעודכן ,newValue - הערך שיש לעדכן בתצוגה לפעמים יהיה חובה שערכו יהיה אך ורק מרשימת האפשרויות שהוגדרה מראש)

 [
  {
    "fieldToUpdate": "opening.0.screen1",
    "userResponse": "הוספתי סרטון למסך 1",
       "newValue": "סרטון"
    }
]
  ...
  - חשוב ביותר !!!!:
   ...
   [
    ...
    },
    {
      ...
      "userResponse": "הוספתי תיאור לסרטון במסך 1",
      "newValue": "סרטון המדגים תופעת טבע של גלי ים..."
    }
   ]
   ...
   [
    {
      ...
      "userResponse": "הוספתי תמונה למסך 2",
      "newValue": "תמונה"
    },
    {
      "fieldToUpdate": "opening.0.screen2Description",
      "userResponse": "הוספתי תיאור לתמונה במסך 2",
      "newValue": "תמונה המציגה מבט על של חוף הים..."
    }
   ]
